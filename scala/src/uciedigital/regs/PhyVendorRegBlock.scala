...
class PhyVendorRegBlock(f: RegFieldTypes, params: UcieRegParams, vIn: PhyToVendor, vOut: VendorToPhy) {
  ...
  private val headerRows: Seq[(Int, Seq[RegField])] = Seq(
    0x0 -> Seq(
      f.HWInit(16, params.vendorId, "vendor_id", "Vendor ID (implementation-specific block)"),
      f.HWInit(16, BlockId, "vendor_id_reg_block", "Vendor ID Register Block PHY")
    ),
    0x4 -> Seq(
    ...
    0x8 -> Seq(
      f.HWInit(32, blockLength, "vendor_reg_block_length",
  ...
  private val overrideRows: Seq[(Int, Seq[RegField])] = {
    ...
    Seq(
      0x10 -> Seq(debugUnlock.field, f.RsvdP(31, "vendor_debug_ctrl_rsvd_31_1")),
      0x14 -> Seq(
        forceActive.field,
        stageSkip.field,
        singleStep.field,
