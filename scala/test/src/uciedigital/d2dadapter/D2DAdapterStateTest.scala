...
/*
  ...
  Coverage:
    ...
    Parity negative:  a REQ_DISABLED injected with a corrupted CP bit is
                      dropped by the sideband RX parity check
                      (SidebandInterfaceNode.scala:100-130): no RSP_DISABLED,
                      no state change; a clean retry afterwards completes the
                      exchange, proving only the corruption was at fault.
 */
class D2DAdapterStateTest extends AnyFunSpec with ChiselSim {
  ...
  private def captureSbMsg(
      dut: D2DAdapter,
      guard: Int = guardCycles
  ): BigInt = {
    assert(
  ...
  private def expectLinkMgmtRsp(
    ...
    assert(
      opcodeOf(msg) == 0x12,
      s"$name must use MessageWithoutData, got opcode 0x${opcodeOf(msg).toHexString}"
    )
    assert(
      msgCodeOf(msg) == msgCode,
      s"$name msgCode mismatch, got 0x${msgCodeOf(msg).toHexString}"
    )
    assert(
      msgSubcodeOf(msg) == msgSubcode,
      s"$name msgSubcode mismatch, got 0x${msgSubcodeOf(msg).toHexString}"
    )
  ...
  describe("D2DAdapter remote-originated disabled/linkReset choreography") {
    ...
    it(
      "answers an injected REQ_DISABLED with RSP_DISABLED, enters disabled, and recovers to reset via both triggers"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        ...
        dut.clock.step(2)
        for (_ <- 0 until 4) {
          dut.io.fdi.plStateSts
            .expect(FDIState.disabled, "disabled must be stable")
          dut.io.fdi.plInbandPres
            .expect(false.B, "inband presence must drop in disabled")
          dut.io.rdi.lpStateReq.expect(
            RDIStateReq.disabled,
            "adapter must request RDI disabled while in disabled"
          )
          dut.clock.step()
        }
        ...
        dut.clock.step(2)
        dut.io.fdi.plStateSts
          .expect(FDIState.reset, "reset must be stable after recovery")
        ...
        dut.clock.step(2)
        dut.io.fdi.plStateSts.expect(
          FDIState.reset,
          "reset must be stable after PHY-triggered recovery"
        )
      }
    }

    it(
      "answers an injected REQ_LINKRESET with RSP_LINKRESET, enters linkReset, and recovers to reset via both triggers"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        ...
        for (_ <- 0 until 4) {
          dut.io.fdi.plStateSts
            .expect(FDIState.linkReset, "linkReset must be stable")
          dut.io.fdi.plInbandPres
            .expect(false.B, "inband presence must drop in linkReset")
          dut.io.rdi.lpStateReq.expect(
            RDIStateReq.linkReset,
            "adapter must request RDI linkReset while in linkReset"
          )
          dut.clock.step()
        }
  ...
  describe("D2DAdapter linkError oscillation") {
    ...
    it(
      "oscillates between reset and linkError every cycle while the PHY holds linkError with RX deactivated"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        dut.clock.step(2)
  ...
  describe("D2DAdapter power-management (unimplemented) negatives") {
    ...
    it(
      "ignores FDI l1/l2 state requests: no state change, no sideband TX, no RDI request"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        dut.clock.step(2)
        ...
        for (req <- Seq(FDIStateReq.l1, FDIStateReq.l2)) {
          ...
          for (_ <- 0 until 16) {
            dut.io.fdi.plStateSts.expect(
              FDIState.reset,
              "PM request must not change the link state"
            )
            dut.io.rdi.lpCfgVld
              .expect(false.B, "PM request must not trigger any sideband TX")
            dut.io.rdi.lpStateReq.expect(
              RDIStateReq.nop,
              "PM request must not generate an RDI state request"
            )
            dut.clock.step()
          }
        }
    ...
    it(
      "drops an injected REQ_L1 sideband message without a response or state change"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        dut.clock.step(2)
        ...
        for (_ <- 0 until 40) {
          dut.io.fdi.plStateSts
            .expect(FDIState.reset, "REQ_L1 must not change the link state")
          dut.io.rdi.lpCfgVld
            .expect(false.B, "REQ_L1 must not be answered on the sideband")
          dut.io.rdi.lpStateReq.expect(
            RDIStateReq.nop,
            "REQ_L1 must not generate an RDI state request"
          )
          dut.clock.step()
        }
      }
  ...
  describe("D2DAdapter sideband parity protection") {

    it(
      "drops a CP-corrupted REQ_DISABLED without a response or state change, then completes a clean retry"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        // Same stability requirement as the disabled test: hold plStateSts
        // away from reset so a successful entry would be observable.
        dut.io.rdi.plStateSts.poke(RDIState.disabled)
        dut.clock.step(2)
        dut.io.fdi.plStateSts.expect(FDIState.reset)

        // Flip the CP bit [62]: the RX parity check must drop the message
        // before it reaches the switch (SidebandInterfaceNode.scala:100-130),
        // so the adapter must neither answer nor change state (D2D-SB-06
        // negative leg).
        injectSbMsg(dut, REQ_DISABLED_MSG ^ (BigInt(1) << 62))
        for (_ <- 0 until 40) {
          dut.io.rdi.lpCfgVld
            .expect(false.B, "a parity-corrupted request must not be answered")
          dut.io.fdi.plStateSts.expect(
            FDIState.reset,
            "a parity-corrupted request must not change the link state"
          )
          dut.clock.step()
        }

        // Positive control: the identical message with correct parity still
        // completes the full {REQ,RSP}_DISABLED exchange into disabled.
        injectSbMsg(dut, REQ_DISABLED_MSG)
        val rsp = captureSbMsg(dut)
        expectLinkMgmtRsp(
          rsp,
          msgCode = 0x04,
          msgSubcode = 0x0c,
          name = "post-corruption RSP_DISABLED"
        )
        assert(
          waitUntil(dut)(fdiStateIs(dut, FDIState.disabled)),
          "a clean retry after the dropped message must still enter disabled"
        )
      }
    }
  }
