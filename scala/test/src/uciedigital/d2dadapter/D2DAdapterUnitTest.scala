...
/*
  Run:
    ./mill --no-daemon test.testOnly edu.berkeley.cs.uciedigital.d2dadapter.D2DAdapterUnitTest
 ...
 */
class D2DAdapterUnitTest extends AnyFunSpec with ChiselSim {
  ...
  describe("D2DAdapter reset semantics") {
    ...
    it(
      "never presents lpIrdy to RDI while the PHY reports reset (9bec310 gating)"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        ...
        for (_ <- 0 until 8) {
          dut.io.rdi.lpIrdy.expect(
            false.B,
            "lpIrdy must stay low while RDI plStateSts == reset"
          )
          dut.clock.step()
        }
      }
  ...
  describe("D2DAdapter adjacent-layer handshakes") {
    ...
    it("acks FDI wake request one cycle later") {
      simulate(mkDut) { dut =>
        ...
        dut.clock.step()
        dut.io.fdi.plWakeAck
          .expect(true.B, "plWakeAck must follow lpWakeReq after 1 cycle")
        dut.io.fdi.lpWakeReq.poke(false.B)
        dut.clock.step()
        dut.io.fdi.plWakeAck.expect(false.B)
      }
    ...
    it("acks RDI clock request one cycle later") {
      simulate(mkDut) { dut =>
        ...
        dut.clock.step()
        dut.io.rdi.lpClkAck
          .expect(true.B, "lpClkAck must follow plClkReq after 1 cycle")
      }
    }
  }
  ...
  describe("D2DAdapter link-error behavior") {
    ...
    it("forwards protocol lpLinkError to RDI with a one-cycle delay") {
      simulate(mkDut) { dut =>
        ...
        dut.clock.step()
        dut.io.rdi.lpLinkError.expect(
          true.B,
          "lpLinkError must reach RDI one cycle after FDI asserts it"
        )
        dut.io.fdi.lpLinkError.poke(false.B)
        dut.clock.step()
        dut.io.rdi.lpLinkError.expect(
          false.B,
          "lpLinkError must clear one cycle after FDI deasserts it"
        )
      }
    }

    it(
      "enters linkError on a PHY linkError report and recovers to reset on an active request"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        dut.clock.step(2)
        ...
        for (_ <- 0 until 4) {
          dut.io.fdi.plStateSts.expect(
            FDIState.linkError,
            "PHY linkError must preempt reset-state bringup"
          )
          dut.io.fdi.plInbandPres
            .expect(false.B, "plInbandPres must stay low during linkError")
          dut.clock.step()
        }
        ...
        dut.io.fdi.plStateSts.expect(
          FDIState.reset,
          "active request with RX deactivated must return the LSM to reset"
        )
      }
    }
  }
  ...
  describe("D2DAdapter bringup behavior") {
    ...
    it(
      "requests RDI active and serializes ADV_CAP once the PHY reports inband presence and active state"
    ) {
      simulate(mkDut) { dut =>
        initDut(dut)
        dut.clock.step(2)
        ...
        dut.io.rdi.lpStateReq.expect(
          RDIStateReq.active,
          "adapter must request RDI active during RDI bringup"
        )
        ...
        dut.io.rdi.lpCfgVld.expect(
          true.B,
          "ADV_CAP must be serialized onto rdi.lpCfg after PARAM_EXCH entry"
        )
        ...
        for (_ <- 0 until 16) {
          dut.io.fdi.plStateSts.expect(
            FDIState.reset,
            "LSM must stay in reset without a remote ADV_CAP"
          )
          dut.io.rdi.lpStateReq.expect(
            RDIStateReq.active,
            "RDI active request must be held in PARAM_EXCH"
          )
          dut.clock.step()
        }
      }
