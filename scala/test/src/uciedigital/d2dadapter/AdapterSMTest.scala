...
class AdapterSMTest extends AnyFunSpec with ChiselSim {
  ...
  private def waitForState(c: AdapterSM, state: RDIState.Type,
                           maxCycles: Int = 60): Unit = {
    var guard = 0
    while (c.io.fdi_pl_state_sts.peek().litValue != state.litValue && guard < maxCycles) {
      c.clock.step()
      guard += 1
    }
  ...
  private def waitUntil(c: AdapterSM, maxCycles: Int = 40)(cond: => Boolean): Unit = {
    var guard = 0
  ...
  /** Wait until the adapter offers the given sideband message, then let the
    * channel consume it.
    */
  private def expectSbAndAccept(c: AdapterSM, msg: UInt, maxCycles: Int = 40): Unit = {
    var guard = 0
  ...
  describe("AdapterSM state exits") {
    ...
    it("Switched from its own Active request to answering the partner's") {
      simulate(dut) { c =>
        ...
        c.io.fdi_lp_state_req.poke(RDIStateReq.active)
        waitUntil(c)(c.io.sb_snd.peek().litValue == SideBandMessage.REQ_ACTIVE.litValue)
        ...
        waitUntil(c)(c.io.sb_snd.peek().litValue == SideBandMessage.RSP_ACTIVE.litValue)
        c.io.sb_rcv.poke(SideBandMessage.NOP)
      }
    }
  ...
  describe("AdapterSM pair") {
    ...
    def waitPair(c: AdapterSMPairHarness, state: RDIState.Type,
                         maxCycles: Int = 200): Unit = {
      var guard = 0
      while (!(c.io.a.fdi_pl_state_sts.peek().litValue == state.litValue &&
    ...
    it("Completed a mutual bringup to active over the sideband link") {
      simulate(new AdapterSMPairHarness(fdiParams, rdiParams, sbParams)) { c =>
        ...
        var guard = 0
        while (!(c.io.a.fdi_pl_inband_pres.peek().litToBoolean &&
