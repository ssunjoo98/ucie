...
class D2DSidebandModuleTest extends AnyFunSpec with ChiselSim {
  ...
  private def sendAtoB(c: D2DSidebandPairHarness, name: String, op: UInt): Unit = {
    c.io.aSnt.poke(op)
    ...
    assert(guard < 60, s"$name: the sideband channel never accepted the message")
    c.clock.step()
    c.io.aSnt.poke(SideBandMessage.NOP)
  ...
  describe("D2DSidebandModule") {
    ...
    it("Carried messages in the reverse direction as well") {
      simulate(harness) { c =>
        ...
        assert(guard < 60, "the sideband channel never accepted die B's message")
        c.clock.step()
        c.io.bSnt.poke(SideBandMessage.NOP)
        ...
        guard = 0
        while (!seen && guard < 120) {
          seen = c.io.aRcv.peek().litValue == SideBandMessage.REQ_ACTIVE.litValue
          c.clock.step()
          guard += 1
        }
    ...
    it("Ran a request/response exchange in both directions") {
      simulate(harness) { c =>
        ...
        guard = 0
        while (!seen && guard < 120) {
          seen = c.io.aRcv.peek().litValue == SideBandMessage.RSP_ACTIVE.litValue
          c.clock.step()
          guard += 1
        }
    ...
    it("Kept D2D link traffic off the protocol-facing FDI port") {
      simulate(harness) { c =>
        ...
        while (!seen && guard < 120) {
          c.io.aFdiPlCfgVld.expect(false.B)
          seen = c.io.bRcv.peek().litValue == SideBandMessage.REQ_ACTIVE.litValue
          c.clock.step()
          guard += 1
        }
