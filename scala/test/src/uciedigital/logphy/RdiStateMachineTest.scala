...
*/
class RdiStateMachineTest extends AnyFunSpec with ChiselSim with UcieSimPrefs {
  ...
  private def expectTxMsg(dut: RDIController, msgCode: Int, msgSubcode: Int,
                          maxCycles: Int = 50): Unit = {
    var cycles = 0
    ...
    assert((data & 0x1f) == OpMsgWithoutData,
      s"TX opcode mismatch: got 0x${(data & 0x1f).toString(16)}, expected MessageWithoutData")
    assert(((data >> 14) & 0xff) == msgCode,
      s"TX msgCode mismatch: got 0x${((data >> 14) & 0xff).toString(16)}, expected 0x${msgCode.toHexString}")
    assert(((data >> 32) & 0xff) == msgSubcode,
      s"TX msgSubcode mismatch: got 0x${((data >> 32) & 0xff).toString(16)}, expected 0x${msgSubcode.toHexString}")
  ...
  private def injectRxMsg(dut: RDIController, msgCode: Int, msgSubcode: Int,
                          maxCycles: Int = 50): Unit = {
    ...
    assert(dut.io.sbLaneIo.rx.ready.peekBoolean(),
      s"DUT never accepted the injected sideband message (msgCode=$msgCode, msgSubcode=$msgSubcode)")
    stepAdapter(dut)
    dut.io.sbLaneIo.rx.valid.poke(false.B)
  }

  private def waitForState(dut: RDIController, state: RDIState.Type,
                           maxCycles: Int = 50): Unit = {
    var cycles = 0
    while (dut.io.rdi.plStateSts.peek().litValue != state.litValue && cycles < maxCycles) {
      stepAdapter(dut)
      cycles += 1
    }
    dut.io.rdi.plStateSts.expect(state, s"RDI state machine never reached $state")
  }
  ...
  private def bringupToActive(dut: RDIController): Unit = {
    ...
    assert(dut.io.rdi.plClkReq.peekBoolean(), "plClkReq must rise for the clock warm-up")
    dut.io.rdi.lpClkAck.poke(true.B)
    dut.clock.step()
    dut.io.cfgSidebandActive.poke(true.B) // now covered: plClkReq is already high
    dut.io.plPhyInRecenter.poke(false.B)

    dut.io.doRdiBringup.poke(true.B)
    dut.io.doingRdiBringup.expect(true.B, "bring-up trigger must report doingRdiBringup")
  ...
  private def expectLocalTransition(dut: RDIController, req: RDIStateReq.Type,
                                    msgSubcode: Int, target: RDIState.Type): Unit = {
  ...
  describe("RDIController Reset -> Active bring-up (LT-RDI-01)") {
    ...
    it("stays in RESET with quiet inputs and no sideband traffic") {
      simulate(mkDut) { dut =>
        ...
        for (_ <- 0 until 8) {
          ...
          dut.io.sbLaneIo.tx.valid.expect(false.B, "no sideband message may be sent in idle RESET")
          dut.clock.step()
        }
      }
    }
    ...
    it("wakes, ungates clocks, exchanges {REQ,RSP}_ACTIVE and reaches ACTIVE") {
      simulate(mkDut) { dut =>
        ...
        dut.clock.step()
        dut.io.ungateClocks.expect(true.B, "a wake request must ungate clocks before the ack")
        dut.io.rdi.plWakeAck.expect(false.B)
        dut.clock.step(3)
        dut.io.rdi.plWakeAck.expect(false.B, "no wake ack while clocks are not stable")
        dut.io.clocksUngatedAndStable.poke(true.B)
        dut.clock.step(2)
        dut.io.rdi.plWakeAck.expect(true.B, "wake ack must follow once clocks are stable")
        ...
        assert(dut.io.rdi.plClkReq.peekBoolean(), "plClkReq must rise before sideband traffic")
        dut.io.rdi.lpClkAck.poke(true.B)
        dut.clock.step()
        dut.io.cfgSidebandActive.poke(true.B)
        dut.io.plPhyInRecenter.poke(false.B)
        ...
        dut.clock.step()
        dut.io.rdi.plInbandPres.expect(true.B, "LINKINIT entry must set plInbandPres")
    ...
    it("completes bring-up via doRdiBringup with the adapter-model helper") {
      simulate(mkDut) { dut =>
        ...
        dut.io.rdi.plInbandPres.expect(true.B, "sACTIVE must keep plInbandPres set")
      }
    }
  }
  ...
  describe("RDIController Active -> Retrain/LinkReset/Disabled (LT-RDI-02)") {
    ...
    it("transitions ACTIVE -> RETRAIN on a local request and back to ACTIVE") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)
        expectLocalTransition(dut, RDIStateReq.retrain, SubRetrain, RDIState.retrain)
        expectReturnToActive(dut)
      }
    }

    it("transitions ACTIVE -> LINKRESET on a local request and back to ACTIVE") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)
        expectLocalTransition(dut, RDIStateReq.linkReset, SubLinkReset, RDIState.linkReset)
        expectReturnToActive(dut)
      }
    }
    ...
    it("transitions ACTIVE -> DISABLED on a local request and back to ACTIVE") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)
        expectLocalTransition(dut, RDIStateReq.disabled, SubDisable, RDIState.disabled)
        expectReturnToActive(dut)
      }
    }

    it("responds to remote REQ_RETRAIN then remote REQ_LINKRESET (responder path)") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)
  ...
  describe("RDIController PM L1/L2 PMNAK rejection (LT-RDI-03)") {
    ...
    it("first local PM request completes a PMNAK round trip; documents that a second one wedges") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)

        var firstExchangeDone = false
        for ((req, sub) <- Seq((RDIStateReq.l1, SubL1), (RDIStateReq.l2, SubL2))) {
    ...
    it("answers a remote REQ_L1 with RSP_PMNAK and settles back in ACTIVE") {
      simulate(mkDut) { dut =>
        ...
        for (_ <- 0 until 6) {
          ...
          assert(legal.contains(s), s"remote REQ_L1 must never leave {active, activePmNak}, saw $s")
          dut.io.rdi.plStallReq.expect(false.B,
  ...
  describe("RDIController trainingTimeout -> linkError (LT-RDI-04)") {
    ...
    it("preempts ACTIVE into linkError on trainingTimeout and recovers to ACTIVE") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)
  ...
  describe("RDIController plInbandPres LTSM tracking (LT-RDI-10)") {
    ...
    it("sets plInbandPres on LT ACTIVE, holds it across training states, and clears it on RESET/TRAINERROR") {
      simulate(mkDut) { dut =>
        initDut(dut)
        dut.clock.step(2)
        ...
        dut.io.cfgSidebandActive.poke(true.B)
        dut.io.plPhyInRecenter.poke(false.B)
        dut.io.rdi.plInbandPres.expect(false.B, "plInbandPres must be low before any LT progress")

        // Set rule: sACTIVE (like sLINKINIT, but without forcing a bring-up
        // exchange) raises inband presence one cycle later.
        dut.io.ltsmState.poke(LTState.sACTIVE)
        dut.clock.step()
        dut.io.rdi.plInbandPres.expect(true.B, "LT ACTIVE must set plInbandPres")

        // Hold rule: intermediate training states neither set nor clear.
        dut.io.ltsmState.poke(LTState.sMBTRAIN)
        for (_ <- 0 until 4) {
          dut.clock.step()
          dut.io.rdi.plInbandPres.expect(true.B, "intermediate LT states must hold plInbandPres")
        }

        // Clear rule: sTRAINERROR drops it.
        dut.io.ltsmState.poke(LTState.sTRAINERROR)
        dut.clock.step()
        dut.io.rdi.plInbandPres.expect(false.B, "LT TRAINERROR must clear plInbandPres")

        // Set again, then clear via sRESET.
        dut.io.ltsmState.poke(LTState.sACTIVE)
        dut.clock.step()
        dut.io.rdi.plInbandPres.expect(true.B, "LT ACTIVE must set plInbandPres again")
        dut.io.ltsmState.poke(LTState.sRESET)
        dut.clock.step()
        dut.io.rdi.plInbandPres.expect(false.B, "LT RESET must clear plInbandPres")
      }
    }
  }
  ...
  describe("RDIController stall trigger conditions (LT-RDI-08)") {
    ...
    it("stalls on a valid framing error while LT is ACTIVE and holds the stall") {
      simulate(mkDut) { dut =>
        bringupToActive(dut)
        dut.io.rdi.plStallReq.expect(false.B)
    ...
    it("documents that the state transition does not wait for lpStallAck") {
      simulate(mkDut) { dut =>
        ...
        assert(dut.io.sbLaneIo.tx.valid.peekBoolean(), "REQ_RETRAIN was never sent")
        val data = dut.io.sbLaneIo.tx.bits.data.peek().litValue
        assert(((data >> 32) & 0xff) == SubRetrain, "expected REQ_RETRAIN encoding")
        dut.clock.step()
        ...
        assert(dut.io.sbLaneIo.rx.ready.peekBoolean(), "RSP_RETRAIN was never accepted")
        dut.clock.step()
        dut.io.sbLaneIo.rx.valid.poke(false.B)

        cycles = 0
        while (dut.io.rdi.plStateSts.peek().litValue != RDIState.retrain.litValue && cycles < 20) {
          dut.clock.step()
          cycles += 1
        }
        ...
        dut.io.rdi.plStateSts.expect(RDIState.retrain,
  ...
  describe("RDIController clock coverage across transitions") {
    ...
    it("documents the fatal assertion when bring-up starts with an idle clock handshake") {
    ...
    it("documents the fatal assertion when cfg sideband activity starts with an idle clock handshake") {
    ...
    it("documents the fatal assertion when a transition starts after clocks were released in ACTIVE") {
