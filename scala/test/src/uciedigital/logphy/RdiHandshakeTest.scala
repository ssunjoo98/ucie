...
*/
class RdiHandshakeTest extends AnyFunSpec with ChiselSim with UcieSimPrefs {
  ...
  describe("RDIClockHandshakeRequester 4-phase handshake (LT-RDI-06)") {
    ...
    it("completes all four phases with a delayed ack") {
      simulate(new RDIClockHandshakeRequester()) { dut =>
        ...
        dut.clock.step()
        dut.io.rdi.plClkReq.expect(true.B, "plClkReq must rise after startHandshake")
        dut.io.ctrl.doneHandshake.expect(false.B)
        ...
        for (_ <- 0 until 3) {
          dut.clock.step()
          dut.io.rdi.plClkReq.expect(true.B, "plClkReq must hold until lpClkAck")
          dut.io.ctrl.doneHandshake.expect(false.B)
        }
        ...
        dut.io.ctrl.doneHandshake.expect(true.B, "doneHandshake must track the ack")
        ...
        dut.io.ctrl.startHandshake.poke(false.B)
        dut.clock.step()
        dut.io.rdi.plClkReq.expect(false.B, "plClkReq must de-assert before lpClkAck (Rule 3)")
        dut.io.ctrl.doneHandshake.expect(false.B)
        dut.io.ctrl.inIdle.expect(false.B)
        ...
        dut.io.rdi.lpClkAck.poke(false.B)
        dut.clock.step()
        dut.io.ctrl.inIdle.expect(true.B, "idle must be reached after the ack drops")
        dut.io.rdi.plClkReq.expect(false.B)
      }
    }
    ...
    it("completes with an immediate (same-observation-cycle) ack") {
      simulate(new RDIClockHandshakeRequester()) { dut =>
        ...
        dut.clock.step()
        dut.io.rdi.plClkReq.expect(false.B, "releaseReq must drop the request before the ack")
        dut.io.rdi.lpClkAck.poke(false.B)
        dut.clock.step()
        dut.io.ctrl.inIdle.expect(true.B)
      }
    ...
    it("aborts a pending request cleanly when released before any ack") {
      simulate(new RDIClockHandshakeRequester()) { dut =>
        ...
        dut.io.ctrl.startHandshake.poke(false.B)
        dut.clock.step()
        dut.io.ctrl.inIdle.expect(true.B, "an unacked request must abort back to idle")
        dut.io.rdi.plClkReq.expect(false.B)
        dut.io.ctrl.doneHandshake.expect(false.B)
      }
    }
  ...
  describe("RDIWakeHandshakeResponder wake handshake and clock ungate (LT-RDI-07)") {
    ...
    it("ungates clocks before acking and orders req/ack edges per the rules") {
      simulate(new RDIWakeHandshakeResponder()) { dut =>
        ...
        dut.clock.step()
        dut.io.ctrl.ungateClocks.expect(true.B, "a wake request must ungate clocks first")
        dut.io.rdi.plWakeAck.expect(false.B)
        ...
        for (_ <- 0 until 3) {
          ...
          dut.io.rdi.plWakeAck.expect(false.B, "no wake ack until clocks are stable")
        }
        ...
        for (_ <- 0 until 3) {
          dut.clock.step()
          dut.io.rdi.plWakeAck.expect(true.B, "the ack must hold while lpWakeReq is high")
          dut.io.ctrl.ungateClocks.expect(true.B)
        }
        ...
        dut.io.rdi.lpWakeReq.poke(false.B)
        dut.clock.step()
        dut.io.rdi.plWakeAck.expect(false.B, "the ack must drop only after the request drops")
        dut.clock.step()
        dut.io.rdi.plWakeAck.expect(false.B)
        dut.io.ctrl.ungateClocks.expect(false.B, "clock ungating ends after the handshake")
      }
    }
    ...
    it("supports back-to-back wake handshakes with already-stable clocks") {
      simulate(new RDIWakeHandshakeResponder()) { dut =>
        ...
        for (round <- 0 until 2) {
          dut.io.rdi.lpWakeReq.poke(true.B)
          dut.clock.step()
          dut.io.rdi.plWakeAck.expect(false.B, s"round $round: ack needs the ungate bubble cycle")
          dut.clock.step()
          dut.io.rdi.plWakeAck.expect(true.B, s"round $round: ack must assert after the bubble")

          dut.io.rdi.lpWakeReq.poke(false.B)
          dut.clock.step(2)
          dut.io.rdi.plWakeAck.expect(false.B, s"round $round: ack must clear after the request drops")
          dut.clock.step()
        }
      }
    }
  ...
  describe("RDIStallRequester 4-phase stall handshake (LT-RDI-08)") {
    ...
    it("runs the full stall sequence with delayed ack and ordered edges") {
      simulate(new RDIStallRequester()) { dut =>
        ...
        dut.clock.step()
        dut.io.rdi.plStallReq.expect(true.B, "plStallReq must rise after startStall")
        dut.io.ctrl.isStalled.expect(false.B, "not stalled before the adapter acks")
        dut.io.ctrl.startStall.poke(false.B) // the FSM must have latched the request
        ...
        dut.clock.step()
        dut.io.ctrl.isStalled.expect(true.B, "isStalled must assert once the ack is seen")
        dut.io.rdi.plStallReq.expect(true.B)
        ...
        dut.clock.step()
        dut.io.rdi.plStallReq.expect(false.B, "plStallReq must fall while lpStallAck is asserted (Rule 3)")
        dut.io.ctrl.isStalled.expect(false.B)
        dut.io.ctrl.inIdle.expect(false.B, "no idle until the ack drops")
        dut.io.ctrl.releaseStall.poke(false.B)
