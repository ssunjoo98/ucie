...
class PatternReaderWithLfsrHarness(afeParams: AfeParams) extends Module {
  ...
  rxLfsr.io.resetLfsr := VecInit(Seq.fill(afeParams.mbLanes)(patternReader.io.rxLfsrCtrl.resetLfsr))
...
class PatternReaderTest extends AnyFunSpec with ChiselSim {
  ...
  val numConsecutive = 16  // Spec: 16 consecutive clean iterations needed to pass.
  val randomScenarios = 20 // constrained-random scenarios per (serRatio, lane count); raise for long regressions.
  ...
  val laneSeeds = Seq(
    BigInt(0x1DBFBC), BigInt(0x0607BB), BigInt(0x1EC760), BigInt(0x18C0DB),
    BigInt(0x010F12), BigInt(0x19CFC9), BigInt(0x0277CE), BigInt(0x1BB807)
  )
  ...
  val allLanes = 3 // remoteFuncLanes "b011": every lane functional, for any lane count.

  def isActive(code: Int, lane: Int): Boolean = ((laneMask(code) >> lane) & 1) == 1
  ...
  def patternWidth(p: PatternSelect.Type): Int =
    p match { case PatternSelect.CLKREPAIR => 48; case PatternSelect.VALTRAIN => 8; case _ => 16 }
  ...
  class RefModel(val serRatio: Int, val lanes: Int) {
    ...
    val perLaneIdWords = Seq.tabulate(lanes) { lane =>
      val pat = (BigInt("1010", 2) << 12) | (BigInt(lane & 0xff) << 4) | BigInt("1010", 2)
      repeatedPatternWords(pat, 16)
    }

    // Cycles to stream a given number of whole iterations.
    def cyclesFor(p: PatternSelect.Type, iterations: Int): Int = iterations * patternWidth(p) / serRatio

    def laneReferenceModels(): Seq[ReferenceLFSR] =
      Seq.tabulate(lanes)(lane => new ReferenceLFSR(laneSeeds(lane % laneSeeds.length), polynomial, lfsrWidth))
    ...
    def driveRxWord(
      ...
      def flip(w: BigInt, corrupt: Boolean): BigInt = if (corrupt) w ^ BigInt(1) else w
      ...
      def laneWord(reference: BigInt, lane: Int): BigInt =
        if (blankData && faulted(lane)) BigInt(0) else flip(reference, corruptData && faulted(lane))
      ...
      patternType match {
        ...
        case PatternSelect.VALTRAIN =>
          ...
          dut.io.mbRxLaneIo.valid.poke(flip(w, (corruptData && faulted(0)) || corruptValid).U)

        case PatternSelect.PERLANEID =>
          dut.io.mbRxLaneIo.valid.poke(flip(valTrainWords(cycle % valTrainWords.length), corruptValid).U)
          for (lane <- 0 until lanes if isActive(code, lane)) {
            val laneWords = perLaneIdWords(logicalLane(code, lane))
            dut.io.mbRxLaneIo.data(lane).poke(laneWord(laneWords(cycle % laneWords.length), lane).U)
          }
    ...
    def stream(
      ...
      for (_ <- 0 until nCycles) {
        driveRxWord(dut, patternType, c, refs, code, corruptData, corruptValid, blankData, faultLanes)
        dut.clock.step()
        if (patternType == PatternSelect.LFSR) refs.foreach(_.advanceState(serRatio))
        c += 1
      }
      c
    }
    ...
    def printResult(dut: PatternReaderWithLfsrHarness, expectedPerLane: Int => Boolean, context: String): Unit = {
      ...
      println(f"[PatternReaderTest] aggregateStatus actual=$agg%-5s expected=${expectedPerLane(0)}%-5s")
      for (lane <- 0 until lanes) {
        val actual = dut.io.interfaceIo.resp.bits.perLaneStatusBits(lane).peek().litToBoolean
        val expected = expectedPerLane(lane)
        val mark = if (actual == expected) "" else "  <-- mismatch"
        println(f"[PatternReaderTest] lane $lane%2d actual=$actual%-5s expected=$expected%-5s$mark")
      }
    }

    def checkResult(dut: PatternReaderWithLfsrHarness, expectedPerLane: Int => Boolean, context: String): Unit = {
      for (lane <- 0 until lanes)
        dut.io.interfaceIo.resp.bits.perLaneStatusBits(lane)
    ...
    def finishAndExpect(dut: PatternReaderWithLfsrHarness, expectedPerLane: Int => Boolean, context: String): Unit = {
      ...
      for (_ <- 0 until 2) { // result holds stable under backpressure
        dut.io.interfaceIo.resp.valid.expect(true.B, s"$context resp.valid held")
  ...
  def startRequest(
    ...
    dut.clock.step()
    dut.io.interfaceIo.req.valid.poke(false.B)
    dut.io.interfaceIo.req.ready.expect(false.B, "request rejected while detecting")
  }
    ...
    for (lanes <- laneCounts; ratio <- serializerRatios) {
      ...
      it(s"consecutive: detects clean CLKREPAIR, VALTRAIN, PERLANEID ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          Seq(PatternSelect.CLKREPAIR, PatternSelect.VALTRAIN, PatternSelect.PERLANEID).foreach { p =>
            startRequest(dut, p, ComparisonMode.PERLANE, errorThreshold = 0, doConsecutiveCount = true, allLanes)
            ref.stream(dut, p, allLanes, ref.cyclesFor(p, numConsecutive), 0, Seq.empty)
      ...
      it(s"consecutive: a mid-stream error forfeits the run ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          Seq(PatternSelect.CLKREPAIR, PatternSelect.VALTRAIN, PatternSelect.PERLANEID).foreach { p =>
            startRequest(dut, p, ComparisonMode.PERLANE, 0, doConsecutiveCount = true, allLanes)
            var c = 0
            c = ref.stream(dut, p, allLanes, ref.cyclesFor(p, 8), c, Seq.empty)
            c = ref.stream(dut, p, allLanes, nCycles = 1, c, Seq.empty, corruptData = true)
            c = ref.stream(dut, p, allLanes, ref.cyclesFor(p, 8), c, Seq.empty)
      ...
      it(s"consecutive: re-counts after an error and still passes ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          startRequest(dut, PatternSelect.PERLANEID, ComparisonMode.PERLANE, 0, doConsecutiveCount = true, allLanes)
          var c = 0
          c = ref.stream(dut, PatternSelect.PERLANEID, allLanes, ref.cyclesFor(PatternSelect.PERLANEID, 8), c, Seq.empty)
          c = ref.stream(dut, PatternSelect.PERLANEID, allLanes, nCycles = 1, c, Seq.empty, corruptData = true)
          c = ref.stream(dut, PatternSelect.PERLANEID, allLanes, ref.cyclesFor(PatternSelect.PERLANEID, numConsecutive + 2), c, Seq.empty)
          val active = ref.activeLanes(PatternSelect.PERLANEID, allLanes)
          ref.finishAndExpect(dut, lane => active.contains(lane), "PERLANEID recount")
        }
      }
      ...
      it(s"consecutive: bad valid framing forfeits the data lanes ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          var c = 0
          c = ref.stream(dut, PatternSelect.PERLANEID, allLanes, ref.cyclesFor(PatternSelect.PERLANEID, 8), c, Seq.empty)
          c = ref.stream(dut, PatternSelect.PERLANEID, allLanes, nCycles = 1, c, Seq.empty, corruptValid = true)
          c = ref.stream(dut, PatternSelect.PERLANEID, allLanes, ref.cyclesFor(PatternSelect.PERLANEID, 8), c, Seq.empty)
      ...
      it(s"error-count: passes within threshold and fails beyond it ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          startRequest(dut, PatternSelect.LFSR, ComparisonMode.PERLANE, errorThreshold = 0, doConsecutiveCount = false, allLanes)
          ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 12, 0, ref.laneReferenceModels())
          ref.finishAndExpect(dut, _ => true, "LFSR clean")

          // (b) One single-bit error per lane with threshold 2 stays within budget.
          startRequest(dut, PatternSelect.LFSR, ComparisonMode.PERLANE, errorThreshold = 2, doConsecutiveCount = false, allLanes)
          val refsB = ref.laneReferenceModels()
          var c = 0
          c = ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 4, c, refsB)
          c = ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 1, c, refsB, corruptData = true)
          c = ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 4, c, refsB)
          ...
          startRequest(dut, PatternSelect.LFSR, ComparisonMode.PERLANE, errorThreshold = 4, doConsecutiveCount = false, allLanes)
          ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 4, 0, ref.laneReferenceModels(), blankData = true)
          ref.finishAndExpect(dut, _ => false, "LFSR beyond threshold")
        }
      }

      it(s"error-count: aggregate mode ORs lane mismatches onto lane 0 ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          idleReq(dut); ref.zeroLanes(dut); dut.clock.step()

          // Clean aggregate: lane 0 (the aggregate result) passes.
          startRequest(dut, PatternSelect.LFSR, ComparisonMode.AGGREGATE, errorThreshold = 0, doConsecutiveCount = false, allLanes)
          ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 12, 0, ref.laneReferenceModels())
          ref.finishAndExpect(dut, _ => true, "AGGREGATE clean")

          // Gross errors: the OR of all lanes lands on lane 0 and exceeds threshold.
          startRequest(dut, PatternSelect.LFSR, ComparisonMode.AGGREGATE, errorThreshold = 4, doConsecutiveCount = false, allLanes)
          ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 4, 0, ref.laneReferenceModels(), blankData = true)
          ref.finishAndExpect(dut, lane => lane != 0, "AGGREGATE error") // only lane 0 carries the aggregate
        }
      }
      ...
      it(s"respects the remote functional lane map ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          laneMapsFor(lanes).foreach { case (label, code) =>
            ...
            startRequest(dut, PatternSelect.PERLANEID, ComparisonMode.PERLANE, 0, doConsecutiveCount = true, code)
            ref.stream(dut, PatternSelect.PERLANEID, code, ref.cyclesFor(PatternSelect.PERLANEID, numConsecutive), 0, Seq.empty)
            ref.finishAndExpect(dut, lane => active.contains(lane), s"PERLANEID lane map $label")
            ...
            startRequest(dut, PatternSelect.LFSR, ComparisonMode.PERLANE, errorThreshold = 0, doConsecutiveCount = false, code)
            ref.stream(dut, PatternSelect.LFSR, code, nCycles = 12, 0, ref.laneReferenceModels())
      ...
      it(s"drives the RX LFSR control handshake ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          dut.io.interfaceIo.req.bits.comparisonMode.poke(ComparisonMode.PERLANE)
          ...
          for (_ <- 0 until 3) {
            ...
            dut.io.rxLfsrCtrl.increment.expect(true.B, "increment while detecting LFSR")
            dut.clock.step()
          ...
          startRequest(dut, PatternSelect.CLKREPAIR, ComparisonMode.PERLANE, 0, doConsecutiveCount = true, allLanes)
          ref.driveRxWord(dut, PatternSelect.CLKREPAIR, 0, Seq.empty, allLanes)
          dut.io.rxLfsrCtrl.increment.expect(false.B, "no increment for CLKREPAIR")
          dut.clock.step()
          ref.finishAndExpect(dut, _ => false, "CLKREPAIR ctrl") // too short to reach 16
        }
      }
      ...
      it(s"per-lane: only the corrupted lanes fail ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          ...
          startRequest(dut, PatternSelect.PERLANEID, ComparisonMode.PERLANE, 0, doConsecutiveCount = true, allLanes)
          ref.stream(dut, PatternSelect.PERLANEID, allLanes, ref.cyclesFor(PatternSelect.PERLANEID, numConsecutive), 0, Seq.empty,
            blankData = true, faultLanes = Some(bad))
          ref.finishAndExpect(dut, lane => active.contains(lane) && !bad.contains(lane), "perlane consecutive")

          // LFSR error-count: the blanked lanes exceed the threshold, the clean lanes stay at 0.
          startRequest(dut, PatternSelect.LFSR, ComparisonMode.PERLANE, errorThreshold = 0, doConsecutiveCount = false, allLanes)
          ref.stream(dut, PatternSelect.LFSR, allLanes, nCycles = 8, 0, ref.laneReferenceModels(),
            blankData = true, faultLanes = Some(bad))
          ref.finishAndExpect(dut, lane => !bad.contains(lane), "perlane error-count")
        }
      }

      it(s"constrained-random per-lane outcomes ($tag)") {
        simulate(new PatternReaderWithLfsrHarness(ref.params)) { dut =>
          val rng = new scala.util.Random(randomSeed + ratio.toLong * 131 + lanes)
          idleReq(dut); ref.zeroLanes(dut); dut.clock.step()

          // Functional lane maps for this width (exclude "none" so there is something to pass).
          val codes = laneMapsFor(lanes).map(_._2).filter(c => ref.activeLanes(PatternSelect.PERLANEID, c).nonEmpty)

          for (_ <- 0 until randomScenarios) {
            val code = codes(rng.nextInt(codes.length))
            val active = ref.activeLanes(PatternSelect.PERLANEID, code)
            val bad = active.filter(_ => rng.nextBoolean()) // random subset of active lanes blanked
            ...
            if (rng.nextBoolean()) {
              ...
              ref.stream(dut, PatternSelect.PERLANEID, code, ref.cyclesFor(PatternSelect.PERLANEID, numConsecutive), 0, Seq.empty,
                blankData = true, faultLanes = Some(bad))
              ref.finishAndExpect(dut, lane => active.contains(lane) && !bad.contains(lane), "random consecutive")
            } else {
              // LFSR error-count: blanked active lanes exceed the threshold; everything else passes.
              val threshold = rng.nextInt(5)
              startRequest(dut, PatternSelect.LFSR, ComparisonMode.PERLANE, threshold, doConsecutiveCount = false, code)
              ref.stream(dut, PatternSelect.LFSR, code, nCycles = 8, 0, ref.laneReferenceModels(),
                blankData = true, faultLanes = Some(bad))
              ref.finishAndExpect(dut, lane => !(active.contains(lane) && bad.contains(lane)), "random error-count")
            }
          }
        }
      }
