...
class SidebandPriorityQueueTest extends AnyFunSpec with ChiselSim with VerilatorCoverage {
  ...
  val classification: Seq[(BigInt, Int)] = {
    ...
    Seq(CompletionWithoutData, CompletionWith32bData, CompletionWith64bData).map(_.litValue -> 1) ++
    Seq(MemoryRead_32b, MemoryWrite_32b, DMSRegisterRead_32b, DMSRegisterWrite_32b,
        ConfigurationRead_32b, ConfigurationWrite_32b, MemoryRead_64b, MemoryWrite_64b,
        DMSRegisterRead_64b, DMSRegisterWrite_64b, ConfigurationRead_64b, ConfigurationWrite_64b)
      .map(_.litValue -> 2) ++
    Seq(ManagementPortMsgWithoutData, ManagementPortMsgWithData,
        BackToBackPriorityPacket, SinglePriorityPacket).map(_.litValue -> 3)
  }

  def mkMsg(opcode: BigInt, tag: BigInt): BigInt = (tag << 64) | opcode

  val printDebugs = false
  def printDebug(msg: String): Unit = if (printDebugs) println(s"[SidebandPriorityQueueTest] $msg")
  ...
  describe("SidebandPriorityQueue") {
    it("dequeues classes in strict priority order") {
      simulate(new SidebandPriorityQueue(msgW, SidebandPriorityQueueDepths())) { c =>
        enq(c, other, 4)
        enq(c, accRequest, 3)
        enq(c, accComplete, 2)
        enq(c, reqResp, 1)
    ...
    it("classifies every opcode into the correct priority class") {
      simulate(new SidebandPriorityQueue(msgW, SidebandPriorityQueueDepths())) { c =>
        for ((op, rank) <- classification) {
          ...
          enq(c, accRequest, 2)
          enq(c, other, 3)
          val expected = Seq[BigInt](0, 1, 2, 3).patch(rank, Seq(BigInt(9)), 0)
          assert(Seq.fill(5)(deqTag(c)) == expected, s"opcode 0x${op.toString(16)} misclassified")
        }
      }
    }
    ...
    it("streams under concurrent enqueue and dequeue without loss or reorder") {
      simulate(new SidebandPriorityQueue(msgW, SidebandPriorityQueueDepths())) { c =>
        ...
        for (i <- 1 to n) {
          ...
          c.io.enq.valid.poke(true.B)
          c.io.enq.ready.expect(true.B)
          if (c.io.deq.valid.peek().litToBoolean) got += (c.io.deq.bits.peek().litValue >> 64)
          c.clock.step()
        }
        c.io.enq.valid.poke(false.B)
    ...
    it("holds the output and loses nothing when dequeue stalls mid-stream") {
      simulate(new SidebandPriorityQueue(msgW, SidebandPriorityQueueDepths())) { c =>
        ...
        c.clock.step()
        c.io.deq.valid.expect(true.B)
        assert((c.io.deq.bits.peek().litValue >> 64) == held, "held output changed under backpressure")
    ...
    it("does not block a high-priority enqueue behind a full low-priority queue") {
      val depths = SidebandPriorityQueueDepths(
        messageRequestOrResponse = 4, regAccessCompletion = 4, regAccessRequest = 4, other = 1)
